...
remove_files(){
    ...
    sudo apt autoclean -y && echo_status_ok || echo_status_error "Échec de apt autoclean"

    echo_status "Suppression automatique des paquets (autoremove)"
    sudo apt autoremove -y && echo_status_ok || echo_status_error "Échec de apt autoremove"

    echo_status "Vidage du répertoire /tmp"
    if [[ -d "$HOME/tmp" ]]; then
        sudo find "$HOME/tmp" -mindepth 1 -delete && echo_status_ok || echo_status_error "Échec suppression de $HOME/tmp"
    else
    ...
    if [[ -d "$HOME/.cache" ]]; then
        sudo find "$HOME/.cache" -mindepth 1 -delete && echo_status_ok || echo_status_error "Échec purge de $HOME/.cache"
    else
    ...
    if [[ -d "$TRASH_PATH/files" ]]; then
        sudo find "$TRASH_PATH/files" -mindepth 1 -delete && echo_status_ok || echo_status_error "Échec vidage $TRASH_PATH/files"
    else
    ...
    if [[ -d "$TRASH_PATH/info" ]]; then
        sudo find "$TRASH_PATH/info" -mindepth 1 -delete && echo_status_ok || echo_status_error "Échec vidage $TRASH_PATH/info"
    else
